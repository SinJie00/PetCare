<template>
  ...
  >
    ...
    <GMapMap :center="strayPost.location" ref="mapRef" :zoom="16" map-type-id="terrain" style="width: 70vw; height: 40rem">
      <GMapCluster>
        ...
        <div class="modal-body">
          <form novalidate>
            ...
            <div class="form-group mt-2 mr-2">
              ...
              <p>Current Location: {{ strayPost.location }}</p>
  ...
  methods: {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
          ...
          geocoder.geocode({ location: latlng }, (results, status) => {
            if (status === 'OK') {
              if (results[0]) {
                this.strayPost.location = { lat, lng };
    ...
    getAddressFromLatLng(lat, lng) {
      ...
      geocoder.geocode({ location: latlng }, (results, status) => {
        if (status === 'OK') {
          if (results[0]) {
            this.strayPost.location = results[0].formatted_address; // Set the address in the strayPost object
          } else {
            console.error('No results found for current location.');
          }
        } else {
