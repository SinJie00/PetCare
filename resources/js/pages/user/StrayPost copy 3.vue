<template>
  <div class="container mt-5 mb-5">
    ...
    <div class="bg-light clearfix">
      <button v-if="isAuthenticated" class="btn btn-primary mb-3 float-right" data-bs-toggle="modal" data-bs-target="#strayPostModal">Add Stray
        Post</button>
  ...
  >
    ...
    <GMapMap :center="strayPost.position" ref="mapRef" :zoom="16" map-type-id="terrain" style="width: 70vw; height: 40rem">
...
export default {
  ...
  mounted() {
    ...
    script.src = `https://maps.googleapis.com/maps/api/js?key=AIzaSyAfrTNx21m1BpOFe12uPZsCof8An3TKutk&libraries=places`;
    script.onload = this.getUserLocation;
  ...
  isAuthenticated() {
    ...
    // You can use your authentication state management logic here
    // For example, if you are using Vuex:
  ...
  methods: {
    getStrayPosts() {
      axios.get('http://localhost:81/api/strayposts')
        .then((response) => {
          ...
          /* this.markers = strayPosts.map((strayPost) => ({
            position: {
              lat: strayPost.latitude,
              lng: strayPost.longitude,
            },
          })); */
          this.markers = [{
          position: {
      ...
      // Push the new stray post to the strayPosts array
        /* this.markers.push(strayPost); */
    ...
    /* populateMarkers() {
    this.markers = this.strayPosts.map((post, index) => ({
      id: index,
      position: {
        lat: post.position.lat,
        lng: post.position.lng
      },
      // You can add more properties to the marker object if needed
    }));
    }, */
    placeMarker(latLng) {
      if (this.marker) {
